...
def run_model(config):
    ...
    if planet_params['include_planets']:
        ...
        for i in range(len(Mp)):
            if chemistry_params["on"]:
                ...
                planets.add_planet(planet_params["implant_time"][i], Rp[i], Mp[i], 0, X_core*Mp[i], X_env)
            else:
    ...
    for ti in times:
        ...
        if transport_params['radial_drift']:
            ...
            if d != 0:
                for count, planet in enumerate(planets):
                    ...
                    planetary_mol_abund.data[:] = np.array(X_cores[count]) #+ np.array(X_envs[planet_count])
                    planetary_atom_abund = planetary_mol_abund.atomic_abundance()

                    axes[0].scatter(planet.R.copy(), planetary_atom_abund.number_abund("C")[-1]/planetary_atom_abund.number_abund("O")[-1], c=c2)

            for count, planet in enumerate(planets):
                axes[2].scatter(planet.R.copy(), np.array(planet.M_core.copy())+np.array(planet.M_env.copy()), c=c2)
        ...
        #    Rs_trans = np.transpose(Rs)
        #    Mcs_trans = np.transpose(Mcs)
        #    Mes_trans = np.transpose(Mes)

        #    for d, item in enumerate(Mcs_trans):
        #        axes[0].loglog(time_keeper, Mcs_trans[i]+Mes_trans[i])
        #    axes[0].set_xlabel("R [AU]")
        #    axes[0].set_ylabel("Earth Masses")
        #    axes[0].set_title("Planet Growth Tracks")
        ...
        if chemistry_params["on"]:
            ...
            axes[3].semilogx(R, atom_abund.number_abund("C")/atom_abund.number_abund("O"), label=f"{t/(2*np.pi*10**6):2f} Myr", linestyle="dashed", color=c4)
            #axes[3].set_xlim(0.7, 300)
            axes[3].set_ylim(0, 1.2)
            axes[3].set_ylabel('[C/O]')
            axes[3].set_xlabel("R [AU]")
            axes[3].set_title("C/O ratio of ice throughout the disk")
            #axes[3].legend()
            ...
            planetesimal_abund = disc._planetesimal.ice_abund.atomic_abundance()
            
            axes[0].semilogx(grid.Rc, planetesimal_abund.number_abund("C")/planetesimal_abund.number_abund("O"), color=c4, linestyle='dashed', label="Planetesimals")

            d+=1

    for planet_count, planet_mass in enumerate(Mcs):
        axes[2].loglog(Rs[planet_count], np.array(Mcs[planet_count])+np.array(Mes[planet_count]), color=colors[planet_count])
        #axes[2].semilogy(time_keeper, np.array(Mcs[planet_count])+np.array(Mes[planet_count]), color=colors[planet_count])
        #axes[2].semilogy(time_keeper, Mdot_tracker[planet_count], color=colors[planet_count])

    axes[2].set_xlabel("R [AU]")
    #axes[2].set_xlabel("Time (Myr)")
    axes[2].set_ylabel("Earth Masses")
    #axes[2].set_ylabel("Mdot [Earth Masses]")
    axes[2].set_title("Planet Growth Tracks")
    axes[2].set_xlim(1e-1, 500)
    axes[2].set_ylim(1e-2, 1e-1)
    #axes[2].set_title("Planet Mass Growth")
    #axes[2].set_title("mass growth rate of planets")
    
    for planet_count, planet_ice_chem in enumerate(X_cores):
        planetary_mol_abund = SimpleCOMolAbund(len(X_cores[0][0]))
        planetary_mol_abund.data[:] = np.array(planet_ice_chem) #+ np.array(X_envs[planet_count])
        planetary_atom_abund = planetary_mol_abund.atomic_abundance()
        ...
        axes[0].semilogx(Rs[planet_count], planetary_atom_abund.number_abund("C")/planetary_atom_abund.number_abund("O"), color=colors[planet_count])

    axes[0].set_xlabel("Radius (AU)")
    axes[0].set_ylim(0, 1.2)
    axes[0].set_xlim(1e-1, 500)
    axes[0].set_ylabel("[C/O]")
    axes[0].set_title("C/O Growth Track of planets over time")

    plt.tight_layout()

    fig.savefig('graphs/chem_w_planets/only_planetesimal_accretion.png', bbox_inches='tight')
...
if __name__ == "__main__":
    config = {
        ...
        "planets": {
            ...
            "planet_model": "Bitsch2015Model",
            "Rp": [1, 3, 5, 10, 30], #[1, 5, 10, 20, 30], # initial position of embryo [AU]

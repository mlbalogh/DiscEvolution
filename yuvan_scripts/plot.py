...
axes[0].set_ylabel("Metallicity [$M_X / M_{tot}$]")
axes[0].set_xlabel("Psi")
axes[0].set_xticks([25, 50, 75, 100], ["0", "0.01", "1", "1000"])

axes[1].set_title("H2O Abundance versus Psi")
axes[1].set_ylabel("H2O Abundance [?]")
axes[1].set_xlabel("Psi")
axes[1].set_xticks([25, 50, 75, 100], ["0", "0.01", "1", "1000"])

axes[2].set_title("C/O ratio of planet versus Psi")
axes[2].set_ylabel("$[C/O]$")
axes[2].set_xlabel("Psi")
axes[2].set_xticks([25, 50, 75, 100], ["0", "0.01", "1", "1000"])

axes[3].set_title("C/O ratio of envelope versus Psi")
axes[3].set_ylabel("$[C/O]_{env}$")
axes[3].set_xlabel("Psi")
axes[3].set_xticks([25, 50, 75, 100], ["0", "0.01", "1", "1000"])

for psi in [0, 0.01, 1, 1000]:
    if psi == 0:
        x = 25
        psi = 0
        subset="visc"
            ...
            for Rd in [10, 50, 100]:
                ...
                with open(f"data/{subset}/{subset}s_mig_psi{psi}_Mdot{Mdot:.1e}_M{M:.1e}_Rd{Rd:.1e}.json", 'r') as i:
                    ...
                    if len(metallicity) != 0:
                        ...
                        axes[0].scatter([x+offset for item in metallicity], metallicity, zorder=0, color=color, s=size, marker=marker)
                        axes[1].scatter([x+offset for item in H2O_abund], H2O_abund, zorder=0, color=color, s=size, marker=marker)
                        axes[2].scatter([x+offset for item in CO_ratio], CO_ratio, zorder=0, color=color, s=size, marker=marker)
                        axes[3].scatter([x+offset for item in CO_env_ratio], CO_env_ratio, zorder=0, color=color, s=size, marker=marker)
...
plt.tight_layout()
fig.savefig("graphs/results/offset_Rd_all_plans.png", bbox_inches="tight")
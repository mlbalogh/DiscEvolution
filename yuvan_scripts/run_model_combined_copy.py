...
def run_model(config):
    ...
    for ti in times:
        ...
        example_radius = []
        example_masses = []
        for mass in np.linspace(1e-2, 1e3, 10000):
            example_radius.append(1)
            example_masses.append(mass)
        
        example_radius = np.array(example_radius)
        example_masses = np.array(example_masses)

        crida_mig = planet_model._migrate.migration_rate(example_radius, example_masses)
        #nekrasov_mig = np.abs(planet_model._migrate.migration_rate_winds(example_radius, example_masses))

        plt.plot(example_masses, crida_mig, color="black", label="Crida")
        #plt.plot(example_masses, nekrasov_mig, color="red", label="Nekrasov")
        plt.legend()
    
        plt.tight_layout()

        plt.savefig('mig_test.png', bbox_inches='tight')
        plt.clf()
        print()
...
if __name__ == "__main__":
    config = {
        ...
        "planets": {
            ...
            "pb_gas_f": 0.1, # Percent of accreted solids converted to gas
            "migrate" : True,

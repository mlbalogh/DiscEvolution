...
from DiscEvolution.constants import Msun, AU, yr,m_p, k_B, GasConst 
from DiscEvolution.grid import Grid
from DiscEvolution.star import SimpleStar
from DiscEvolution.eos  import IrradiatedEOS, LocallyIsothermalEOS, SimpleDiscEOS, ChambersEOS
...
from DiscEvolution.driver import DiscEvolutionDriver, PlanetEvolutionDriver
from DiscEvolution.io import Event_Controller, DiscReader
from DiscEvolution.disc_utils import mkdir_p,debug_pause
...
from DiscEvolution.planet_formation import (Planets, PebbleAccretionHill, Bitsch2015Model,
	TypeIMigration, TypeIIMigration, CridaMigration)
# from DiscEvolution.planet_formation import (PebbleAccretionHill, Bitsch2015Model,
# 	TypeIMigration, TypeIIMigration, CridaMigration)
...
def setup_disc(model):
    ...
    if p['type'] == 'irradiated':
        ...
        eos = ChambersEOS(star, params['sigma0'], params['r0'], params['T0'],
                    params['v0'], params['fw'],
                    params['K'], params['Tevap'],
                    params['rexp'], params['k0'])
    else:
    ...
    if p['type'] == 'Booth-alpha':
        ...
        wind = DiskWindEvolution(star, params['sigma0'], params['r0'], params['T0'],
                    ...
                    params['K'], params['Tevap'],
                    params['rexp'], params['k0'])
        if params['alpha_turb'] and params['alpha_wind'] is not None:
            wind.calculate_chambers_params(params['alpha_turb'], params['alpha_wind'], set=True)
            try: 
...
def setup_planets(model,disc):
    # planets=[]
    # for i in np.arange(np.size(model['planets'])):
    #     Mp=model['planets'][i]['Mp']
    #     ap=model['planets'][i]['R']
    #     planets.append(Planet(Mp, ap)) 
    # planet_model = Bitsch2015Model(disc)
    # return PlanetList(planets), planet_model
    planets = Planets()
...
def setup_model(model, disc, planets, planet_model, start_time):
    ...
    if model['transport']['gas']=="viscous":
        gas = ViscousEvolution()
    if model['transport']['gas'] == 'Chambers' or model['transport']['gas'] == "chambers":
        params = model['chambers params']
        gas = DiskWindEvolution(disc.star, params['sigma0'], params['r0'], params['T0'],
                    params['v0'], params['fw'],
                    params['K'], params['Tevap'],
                    params['rexp'], params['k0'])
        if params['alpha_turb'] and params['alpha_wind'] is not None:
            gas.calculate_chambers_params(params['alpha_turb'], params['alpha_wind'], set=True) 
    ...
    return PlanetEvolutionDriver(disc, 
...
def setup_output(model):
    ...
    return base_name, base_planet_name, EC
...
def main(*args):
    ...
    driver = setup_model(model, disc, planets, planet_model, time)

    output_name, base_planet_name, io_control = setup_output(model)
    #debug_pause()

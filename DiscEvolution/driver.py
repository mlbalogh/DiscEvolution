...
class PlanetEvolutionDriver(object):
    """Driver class for full evolution model.  

    Author: MLB Feb 2025 modification of Booth's DiscEvolutionDriver
    ...
    Optional Physics update:
        ...
        planets          : Update planet mass and radius
    ...
    def __init__(self, disc, gas=None, dust=None, planets=None, planet_model=None, diffusion=None, chemistry=None, ext_photoevaporation=None, int_photoevaporation=None, history=None, t0=0.):
    ...
    def __call__(self, tmax):
        """Evolve the disc and planet for a single timestep
        ...
        planet_model = self._planet_model
        ...
        if self._dust:
            ...
            # print("dust dt:", self._dust.max_timestep(self._disc, v_visc))
        ...
        #vr=self._gas.viscous_velocity(disc)
        #Mdot_actual = disc.Mdot(vr)
        #print (self.t,Mdot_actual[0:5],disc.T[0:5])
        #debug_pause()
        disc.update(dt)
        #planet_model.update()
        # Do planet evolution
        if self._planet_model:
            # Is this update needed?  I think integrate does the update
            #self._planet_model.update()
            self._planet_model.integrate(dt, planets)
            #Mdot_s = self._planet_model._peb_acc.computeMdot(planets.R, planets.M_core + planets.M_env)
        self._t += dt
        self._nstep += 1
        return dt
    ...
    # def dump_planets_ASCII(self, filename):
    #     head = ''
    #     io.dump_planets_ASCII(filename, self._disc, self._planet_model, self._planets, self.t, head)
